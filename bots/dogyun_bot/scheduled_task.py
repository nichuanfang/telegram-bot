...
# 每月7号
async def get_traffic_packet(context: CallbackContext):
    ...
    try:
        ...
        await context.bot.send_message(DOGYUN_BOT_CHAT_ID, f'领取失败: {e}')
        return
    # 获取当前时间
    now = datetime.now()
    # 获取当前日期
    ...
    # 发送通知
    await context.bot.send_message(DOGYUN_BOT_CHAT_ID, f'等级奖励通用流量包: {result}')


async def lucky_draw_notice(context: CallbackContext):
    """抽奖活动通知
    """
    ...
    try:
        ...
        if result is not None:
            await context.bot.send_message(DOGYUN_BOT_CHAT_ID, f'抽奖活动通知: {soup.find("strong").text}')
...
# 余额不足提醒
async def balance_lack_notice(context: CallbackContext):
    """余额不足提醒
    """
    ...
    try:
        ...
        if float(balance) < 10:
            await context.bot.send_message(DOGYUN_BOT_CHAT_ID, f'余额不足提醒: {balance}元')
